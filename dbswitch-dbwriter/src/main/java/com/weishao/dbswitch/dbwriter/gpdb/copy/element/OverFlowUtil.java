...
public final class OverFlowUtil {
	...
	public static void validateLongNotOverFlow(final BigInteger integer) {
		...
		if (isOverFlow) {
			throw new RuntimeException(String.format("[%s] 转为Long类型出现溢出 .", integer.toString()));
		}
	}
	...
	public static void validateDoubleNotOverFlow(final BigDecimal decimal) {
		...
		if (isOverFlow) {
			throw new RuntimeException(String.format("[%s]转为Double类型出现溢出 .", decimal.toPlainString()));
		}
	}
}
